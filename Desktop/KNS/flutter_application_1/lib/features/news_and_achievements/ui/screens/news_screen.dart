...
class _NewsScreenState extends State<NewsScreen> with TickerProviderStateMixin {
  ...
  // news2-api-ipدالة لجلب نص الشريط الإخباري الثاني من قاعدة البيانات (NEWS2)
  Future<String> _fetchNews2() async {
    try {
      final response =
          await http.get(Uri.parse('https://app-alamarah.com/api/news2'));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          // جلب بيانات الشريط المتحرك ديناميكياً من قاعدة البيانات عبر API
          FutureBuilder<String>(
            future: _fetchNews2(),
            builder: (context, snapshot) {
              String rawNewsText = 'جاري تحميل الإعلانات...';
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                rawNewsText = snapshot.data!;
              }
